...
logger = setup_logger('rest_kvstore_migrate')
...
def perform_migration(local_dir, coll_name, sessionKey, id=None, file_ids=None):
    ...
    logger.info('perform_migration: from folder %s to coll_name %s', (local_dir, coll_name))
    logger.info('perform_migration: file_ids %s' % file_ids)
    for file_id in file_ids:
        json_data = readAsJson(os.path.join(local_dir, file_id))       
        if not id:
        ...
        try:
            serverResponse, serverContent = kv_store_rest_request( coll_name, 'POST', sessionKey, False, 
                                                                                        json_data)
            ...
            logger.exception(e)
            logger.error('perform_migration: migrating failed for %s to coll_name %s' % (file_id, coll_name))
            return False
...
def migrate(sessionKey):
    ...
    if os.path.exists(local_streams_dir) :
        ...
        if perform_migration(local_streams_dir, streams_kv_store_coll, sessionKey):
    ...
    if os.path.exists(local_capture_addresses_dir):
        ...
        if perform_migration(local_capture_addresses_dir, misc_kv_store_uri, sessionKey):
    ...
    if os.path.exists(local_file_server_mount_points_dir):
        ...
        if perform_migration(local_file_server_mount_points_dir, file_server_mount_points_kv_store_coll, sessionKey):
    ...
    if os.path.exists(local_stream_forwarder_groups_dir):
        ...
        if perform_migration(local_stream_forwarder_groups_dir, stream_forwarder_groups_kv_store_coll, sessionKey):
    ...
    if os.path.exists(local_users_dir):
        ...
        if perform_migration(local_users_dir, misc_kv_store_uri, sessionKey, 'usertours'):
    ...
    if os.path.exists(os.path.join(local_dir, 'apps')): 
        ...
        if perform_migration(local_dir, misc_kv_store_uri, sessionKey,  'appsmeta', ['apps']):
