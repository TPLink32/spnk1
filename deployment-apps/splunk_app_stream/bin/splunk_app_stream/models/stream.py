...
def delete_from_streams_collection(stream_id):
    global app_streams, stream_app_location_map, dateLastUpdated, lock, stream_forwarder_results_cache
    lock.acquire()  
    try:
...
def init_streams_collection(session_key=None):
    ...
    if run_once:
        if use_kv_store:
            ...
            if session_key:
                (updated, old_version, new_version) = upgrade_streams.copy_default_to_local_streams(use_kv_store, streams_kv_store_coll, session_key)
                if upgrade_streams.modified_streams_exist(use_kv_store, streams_kv_store_coll, session_key):
                    upgrade_streams.update_streams(use_kv_store, old_version, new_version, streams_kv_store_coll, session_key)
        else:
...
def read_streams_from_kv_store(session_key):
    tmp_streams = []
    local_streams = read_from_kv_store_coll(streams_kv_store_coll, session_key, True)
    ...
    for stream in local_streams:
        ...
        if 'expirationDate' in stream:
            ...
            if dt.fromtimestamp(stream['expirationDate']) < dt.now():
                ...
                delete_stream(stream['_key'], session_key)
                stream_expired = True
...
def save_stream_to_kv_store(stream_id, json_data, session_key):
    return save_to_kv_store(streams_kv_store_coll, stream_id, json_data, session_key)

# Delete is only allowed for all streams which are stored
# under local/streams in the stream app.
def delete_stream(stream_id, session_key=None):
    ...
    if use_kv_store:
        try:
            serverResponse, serverContent = kv_store_rest_request(streams_kv_store_coll + '/' + stream_id,
                                                                  'DELETE', session_key)
    ...
    try:
        ...
        try:            
            ...
            if run_once and not update_app_last_date_time and 'local' in stream_path:
                if is_file_modified(stream_path, dateLastUpdated):
        ...
        logger.error("IOerror, unable to read file") 
        logger.exception(ex)  
...
def is_valid_stream_definition(stream_json):
    validator = Draft4Validator(stream_schema)
    error_messages = []
...
class Stream:
    ...
    def list(session_key, id='', **kwargs):
        ...
        if not repository and not id:
            ...
            stream_forwarder_id_param_supplied = kwargs.has_key('streamForwarderId')
        ...
        if is_valid_stream:
            ...
            if not found_stream:
                ...
                if include_in_defaultgroup == 'true':
                    logger.info(default_group_json['streams'])
