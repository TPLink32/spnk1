...
class TestCodec():
    ...
    def clusterer_util(self, clusterer):
        ...
        assert model_orig.predict(X[0]) == model_decoded.predict(X[0])
    ...
    def transformer_util(self, transformer):
        ...
        j = json.dumps(model_orig, cls=MLSPLEncoder)
        model_decoded = json.loads(j, cls=MLSPLDecoder)
        assert (model_orig.transform(X[0]) == model_decoded.transform(X[0])).all()
    ...
    def selector_util(self, estimator, X, y):
        ...
        j = json.dumps(model_orig, cls=MLSPLEncoder)
        model_decoded = json.loads(j, cls=MLSPLDecoder)
        assert model_orig.transform(X[0]) == model_decoded.transform(X[0])
