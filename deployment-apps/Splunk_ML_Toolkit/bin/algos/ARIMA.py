...
class ARIMA(BaseAlgo):
    ...
    def __init__(self, options):
        ...
        if 'order' in params:
            ...
            try:
                ...
                raise RuntimeError('Syntax Error: order requires three non-negative integer values, e.g. order=4-1-2')
        else:
    ...
    def handle_options(self, options):
        ...
        if len(self.feature_variables) == 2:
            if '_time' in self.feature_variables:
                self.time_series = (set(self.feature_variables) - set(['_time'])
                                    ).pop()
            else:
    ...
    def _find_freq(X, threshold):
        ...
        err = ('Sampling is irregular. ARIMA will use the median '
               'time interval: {} '
        ...
        timestr = "{:0>8}".format(datetime.timedelta(seconds=median))
        if ratio < threshold:
    ...
    def _forecast(self, options, output_name=None):
        ...
        lower_name = 'lower%d(%s)' % (alpha_to_confidence_interval(options['alpha']), output_name)
        upper_name = 'upper%d(%s)' % (alpha_to_confidence_interval(options['alpha']), output_name)
    ...
    def fit(self, df, options=None):
        ...
        if self.holdback > 0:
            if self.holdback >= len(df):
                raise RuntimeError('holdback value ({}) must be less than the number of events ({})'.format(
