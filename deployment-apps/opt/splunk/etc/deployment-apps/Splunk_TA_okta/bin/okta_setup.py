...
logger = log.Logs().get_logger('ta_okta', level=logging.DEBUG)
...
class ConfigApp(admin.MConfigHandler):
    ...
    def setup(self):
        if self.requestedAction == admin.ACTION_EDIT:
            for arg in ['proxy_enabled', 'proxy_type', 'proxy_url', 'proxy_port',
                'proxy_username', 'proxy_password', 'proxy_rdns', 'loglevel', 'custom_cmd_enabled', 'okta_server_url','okta_server_token']:
    ...
    def handleList(self, confInfo):
        ...
        if confDict is not None:
            ...
            confInfo["okta_loglevel"].append('loglevel', \
                confDict.get('okta_loglevel', {}).get('loglevel', "WARN"))
    ...
    def _decrypt_username_password(self, confDict):
        try:
            if confDict.get("okta_proxy") is not None:
                account = confDict["okta_proxy"]
                is_encrypted = all(account.get(k, None) == \
                    self.encrypted for k in ("proxy_username", "proxy_password"))
                if is_encrypted:
                    cred_mgr = cred.CredentialManager(session_key=self.getSessionKey(),
                        app=self.appName, splunkd_uri=scc.getMgmtUri())
                    user_creds = cred_mgr.get_clear_password("__Splunk_TA_okta_proxy__")
                    account["proxy_username"] = user_creds["__Splunk_TA_okta_proxy__"]["proxy_username"]
                    account["proxy_password"] = user_creds["__Splunk_TA_okta_proxy__"]["proxy_password"]
        except Exception:
