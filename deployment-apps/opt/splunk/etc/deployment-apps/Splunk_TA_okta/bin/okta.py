...
def get_okta_modinput_configs(modinputs):
    try:
        ...
        if input_conf.get("metrics", "event")=="event" and not input_conf.get("start_date", ""):
            input_conf["start_date"] = (datetime.utcnow()-timedelta(days=30)).strftime("%Y-%m-%dT%H:%M:%S.000Z")
            config.update_input_conf(input_name, input_conf)
        ...
        _LOGGER.error("Failed to setup config for okta TA: %s", ex.message)
...
def validate_config():
    ...
    config = okta_config.OktaConfig(input_config.get("server_uri"),
                input_config.get("session_key"), input_config.get("checkpoint_dir"))
    stanza_name = input_config.get("name").replace("okta://", "").strip()
    token = input_config.get("token")
    encrypted_token = config._get_encrypted_token(stanza_name, check_exist=False) or ""
    if token == config.encrypted_display_str and not encrypted_token:
        log_and_raise_value_exception("Please input a correctly formatted token!")
