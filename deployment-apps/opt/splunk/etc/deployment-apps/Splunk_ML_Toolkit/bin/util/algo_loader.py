...
class AlgoLoader(object):
    """
    Class to use for loading algorithm list with the given algo loading strategy.
...
class RestAlgoLoadingStrategy(AlgoLoadingStrategy):
    """
    Load algorithm configuration using the Splunk REST API.

    """
    def __init__(self, searchinfo):
        self.proxy = rest_proxy_from_searchinfo(searchinfo)

    def get_algos_from_splunk(self):
        """
        Get the raw algorithm list from the Splunk REST URL.
        ...
        """
        url = '%s/%s/nobody/%s/configs/conf-algos?output_mode=json&count=-1' % (
            self.proxy.splunkd_uri,
            self.proxy.name_space_str,
            self.proxy.splunk_app,
        )
    ...
    def load_algos(self):
        ...
        """
        resp = self.get_algos_from_splunk()
        if not resp:
...
class BtoolAlgoLoadingStrategy(AlgoLoadingStrategy):
    ...
    def __init__(self, searchinfo):
        self.proxy = BtoolProxy(
            users_or_roles=searchinfo['roles'],
            app=searchinfo['app'],
            target_dir=searchinfo['bundle_path']
        )
