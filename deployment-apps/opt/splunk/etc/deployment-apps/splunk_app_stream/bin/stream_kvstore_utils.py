...
def is_splunkd_ready(sessionKey=None):
    ...
    logger.info("is_splunkd_ready: splunk_ready :: %s, splunkd_fatal_error :: %s, kvstore_fatal_error :: %s" % (splunkd_ready, splunkd_fatal_error, kvstore_fatal_error))
...
def is_kv_store_supported_in_splunk(sessionKey=None):
    ...
    if splunk_version > 630:
        ...
        logger.info("is_kv_store_supported_in_splunk kvStore Status :: %s" % kvStoreStatus)
...
def is_kv_store_ready(sessionKey=None):
    ...
    logger.info('splunk fatal error: %s kv store fatal error: %s', (splunkd_fatal_error, kvstore_fatal_error))
    logger.info('is_kv_store_ready, kv store status :: %s', kvStoreStatus)
    logger.info('search_head_shc_member:: server_roles %s' % server_roles)
...
def kv_store_rest_request(uri, method, sessionKey=None, repository=False, req_data=None):
    ...
    try:
        ...
        result = splunk.rest.simpleRequest(
            url_internal,
            sessionKey,
            postargs=None,
            method=method,
...
def read_from_kv_store_coll(coll_name, session_key, repository=False):
    try:
        ...
        logger.error("read_from_kv_store_coll: REST Exception Caught when invoking REST API:: %s, collection %s" % (str(re), coll_name))
        return []
...
def read_by_id_from_kv_store_coll(coll_name, id, session_key, repository=False):
    try:
        ...
        logger.error("read_by_id_from_kv_store_coll: REST Exception Caught when invoking REST API:: %s, collection %s, id %s" % (str(re), coll_name, id))
...
def update_kv_store_apps_meta(session_key, create=False):
    ...
    try:
        ...
        logger.error("update_kv_store_apps_meta: REST Exception Caught when invoking REST API :: %s" % str(re))
        return {}
...
def read_kv_store_apps_meta(session_key):
    ...
    try:
        ...
        logger.error("read_kv_store_apps_meta: REST Exception Caught when invoking REST API :: %s" % str(re))
        return serverContent
...
def save_to_kv_store(coll_name, id, json_data, session_key):
    try:
        ...
        logger.error("save_to_kv_store: REST Exception Caught when invoking REST API :: %s" % str(re))
        return False
