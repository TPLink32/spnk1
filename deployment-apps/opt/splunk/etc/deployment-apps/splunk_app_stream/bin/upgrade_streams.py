...
logger = setup_logger('upgrade_streams')
...
oob_stream_default_disabled_fields = ['flow_id', 'protocol_stack', 'result_column_count', 'result_row_count', 'sql_error_code', 'bind_variable_value']
...
def copy_default_to_local_streams(use_kv_store, coll_name=None, session_key=None):
    ...
    if os.path.exists(base_local_dir):
        ...
        old_version_json = readAsJson(version_file)
        if old_version_json == 'NotFound':
            # read again from the path that was used in old versions
            old_version_json = readAsJson(os.path.join(base_local_streams_dir, '.version'))
        local_stream_ids = []
        if use_kv_store:
            local_streams = read_from_kv_store_coll(coll_name, session_key, True)
        ...
        if old_version_json != 'NotFound':
            ...
            logger.info('old version found %s' % old_version)
            ...
            for s_id in new_streams_to_copy:
                if s_id not in local_stream_ids:
                    ...
                    if use_kv_store:
                        serverResponse, serverContent = kv_store_rest_request(coll_name + '/' + s_id, 'DELETE', session_key)
                try:
                    if use_kv_store:
                        stream_json = readAsJson(os.path.join(base_default_streams_dir, s_id))
                        stream_json['_key'] = stream_json['id']
                        serverResponse, serverContent = kv_store_rest_request( coll_name, 'POST', session_key, False,
                                                                                stream_json)
                    else:
                        shutil.copy2(os.path.join(base_default_streams_dir, s_id), base_local_streams_dir)
                    updated = True
                except:
                    logger.error("Error copying the file %s" % os.path.join(base_default_streams_dir, s_id))
                    pass
        else:
...
def modified_streams_exist(use_kv_store, coll_name=None, session_key=None):
    ...
    if use_kv_store:
        ...
        local_stream_ids = get_stream_ids(base_local_streams_dir)
    ...
    for stream_id in local_stream_ids:
        ...
        try:
            ...
            if use_kv_store:
                ...
                local_stream = readAsJson(stream_path)
            ...
            if local_stream != "NotFound":
                ...
                default_stream = readAsJson(default_stream_path)
                ...
                if stream_id != default_stream_id and stream_id in default_stream_ids:
                    oob_stream = readAsJson(os.path.join(base_default_streams_dir, stream_id))
                ...
                if stream_modified:
                    ...
                    if use_kv_store:
                        ...
                        serverResponse, serverContent = kv_store_rest_request(coll_name + '/' + local_stream['id'],
                                                                                'PUT', session_key, False, local_stream)
                    else:
                        writeAsJson(stream_path, local_stream)
