...
user_tours_kv_store_with_session_key_uri = misc_kv_store_uri + '/usertours'
user_easy_setup_kv_store_uri = '/servicesNS/nobody/splunk_app_stream/storage/collections/data/users/all/easysetup'
user_easy_setup_kv_store_with_session_key_uri = misc_kv_store_uri + '/easysetup'
use_kv_store = is_kv_store_supported_in_splunk()
if not use_kv_store:
    createDir(users_dir + os.sep)
...
def init_users(session_key=None):
    ...
    try:
        ...
        if pingData['dateLastUpdated'] > dateLastUpdated:
            ...
            if not use_kv_store:
                tour_users_list = readAsJson(os.path.join(users_dir, 'tour_users'))
                if tour_users_list != 'NotFound':
                    tour_users = set(tour_users_list)
                easy_setup_users_list = readAsJson(os.path.join(users_dir, 'easy_setup_users'))
...
def update(user_flag, session_key):
    if use_kv_store:
        ...
        if not save_to_kv_store(misc_kv_store_uri, flag_id, json_data, session_key):
            save_to_kv_store(misc_kv_store_uri, None, json_data, session_key)
    else:
        if user_flag == 'tour':
            writeAsJson(os.path.join(users_dir, 'tour_users'), list(tour_users))
        else:
            writeAsJson(os.path.join(users_dir, 'easy_setup_users'), list(easy_setup_users))
