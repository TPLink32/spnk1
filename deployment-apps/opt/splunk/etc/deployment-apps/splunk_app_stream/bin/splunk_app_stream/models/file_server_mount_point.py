...
logger = setup_logger('fileservermountpoint')
...
class FileServerMountPoint:
    ...
    def list(session_key=None, id='', **kwargs):
        ...
        if use_kv_store and not hide_kv_store(kwargs):
            uri = file_server_mount_points_kv_store_coll
            ...
            if id:
                ...
                response = read_by_id_from_kv_store_coll(uri, id, session_key)
                    ...
                    if os.path.exists(config_dir):
                        for fname in os.listdir(config_dir):
                            ...
                            try:
                                ...
                                f.close()
                return mount_point_dict.values()
    ...
    def create(json_data, session_key=None, **kwargs):
        ...
        if is_valid(json_data):
            if use_kv_store and not hide_kv_store(kwargs):
                ...
                try:
                    save_succeeded = save_to_kv_store(uri, None, json_data, session_key)
                    if save_succeeded:
                        return json_data
                    else:
        ...
        if is_valid(json_data):
            if use_kv_store and not hide_kv_store(kwargs):
                ...
                try:
                    ...
                    update_succeeded = save_to_kv_store(uri, id, json_data, session_key)
                    if update_succeeded:
                        return json_data
                    else:
    ...
    def delete(id='', session_key=None, **kwargs):
        if use_kv_store and not hide_kv_store(kwargs):
            ...
            if id:
                # The 2nd param is needed so that '/' is also encoded.
                uri = uri + '/' + urllib.quote(id, '')
            try:
                serverResponse, serverContent = kv_store_rest_request(uri, 'DELETE', session_key)
