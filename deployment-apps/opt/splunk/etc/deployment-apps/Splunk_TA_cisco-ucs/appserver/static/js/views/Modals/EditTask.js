    ...
    return Backbone.View.extend({
        ...
        render: function() {
            ...
            this.$("[role=dialog]").on('shown.bs.modal', function() {
                $("#addon_edit_task_dialog_server_select_panel_table").bootstrapTable({
                    url: splunkdUtils.make_url('custom/Splunk_TA_cisco-ucs/manage_servers/servers')
                });

                $("#addon_edit_task_dialog_template_select_panel_table").bootstrapTable({
            ...
            this.initServerSelectionsUtil("servers", "#addon_edit_task_dialog_server_select_panel_table");
            this.initServerSelectionsUtil("templates", "#addon_edit_task_dialog_template_select_panel_table");
        ...
        initServerSelectionsUtil: function(modelPropName, id) {
            ...
            var items = this.convertToString(this.model.get(modelPropName)).split("|");
            var nameAppDict = {};
            var itemName = "";
            var appName = "";
            ...
            for (var i = 0; i < rows.length; ++i) {
                if (rows[i].name in nameAppDict)
                {
                    if (rows[i].appName == undefined)
                        rows[i].appName = "Splunk_TA_cisco-ucs";
        ...
        editTask: function() {
            ...
            var edit_task_val = {
                ...
                "servers" : this.getSelectionListSelections("#addon_edit_task_dialog_server_select_panel_table"),
                "templates" : this.getSelectionListSelections("#addon_edit_task_dialog_template_select_panel_table")
             };
