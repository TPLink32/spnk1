...
if __name__ == "__main__":
    ...
    try:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("%s", json.dumps(modaction.settings, sort_keys=True, indent=4, separators=(',', ': ')))
            modaction.addevent("Settings are: %s" % json.dumps(modaction.settings, sort_keys=True, indent=4, separators=(',', ': ')), modaction.action_name)
