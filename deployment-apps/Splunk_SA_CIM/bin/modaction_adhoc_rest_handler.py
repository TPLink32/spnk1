...
def setup_logger(name, level=logging.INFO, maxBytes=25000000, backupCount=5, fmt=SHORT_FORMAT):
    """
    ...
    handler_exists = any([True for h in loginst.handlers if h.baseFilename == logfile])
...
class ModularActionAdhocRestHandler(PersistentServerConnectionApplication):
    ...
    def get_action(cls, action, session_key):
        ...
        :return A dictionary representing the alert action configuration, retrieved from REST.
        :rtype dict
        """
        ...
        if splunk.util.normalizeBoolean(cam_params.get('supports_adhoc', False)):
            ...
            raise ModularActionAdhocException('Alert action does not support adhoc invocation.')
    ...
    def replace_tokens(cls, inputstr, tokens, replacement='', kv=False):
        ...
        :param kv: A Boolean governing the behavior of the token replacement on key=value pairs (see below)
        :type kv: bool

        :return A Splunk search string with tokens replaced.
        :rtype str

        Two replacement modes are provided based on the value of the "kv" Boolean:

        If kv==True, tokens in this form are REMOVED ENTIRELY from the string. This is used to scrub
        key=value pairs from a search string when the subject of the replacement is unavailable.
        Whitespace around the equals sign is permitted in this replacement mode.
        ...
        Only the token itself is replaced in this mode; leading and trailing quotes are unaltered.
        """
    ...
    def replace_standard_tokens(cls, inputstr):
        """Replace tokens that are either always unavailable or that have a constant value when a modular action is
    ...
    def replace_event_tokens(cls, inputstr, replacement_map, substitute_defaults=False):
        ...
        :param inputstr: A Splunk search string.
        :type inputstr: str
        :param replacement_map: A dictionary of key=value replacements to be conducted.
        :param substitute_defaults: Whether or not do substitute tokens with defaults specified.
    ...
    def inject_tokens(cls, action_name, inputstr, inject_map):
        """Inject tokens received from user input into the sendalert clause of the search string.
        If a token is already present in the sendalert clause, it will NOT be re-inserted,
        since that would cause duplicate arguments to be passed to the "sendalert" command.

        :param action_name: The modular action name.
        :type action_name: str
        :param inputstr: A string representing a Splunk search, which is expected to contain "sendalert".
        :type inputstr: str
        :param inject_map: A dictionary containing key=value token pairs to be injected.
        ...
        clause_rx = re.compile(
            r"sendalert\s+(?P<action_name>%s)\s*(?P<args>[^|]*)(?:\||\s*$)" % action_name)
        ...
        if clause_match:
            ...
            cleaned_inject_map = {k: v for k, v in tokens.iteritems() if k not in token_matches}
            inject_str = " ".join(
                [k + "=" + cls.quote_value(v) for k, v in cleaned_inject_map.iteritems()])

            if inject_str:
                components = (i for i in [inputstr[0:clause_match.span(1)[1]].strip(),
                                          inject_str,
                                          inputstr[clause_match.span(1)[1]:].strip()] if i != '')
                return " ".join(components)
            else:
                return inputstr
        else:
            # Should never get here.
            raise ModularActionAdhocException('Mismatch between modular action name and sendalert clause.')

    @classmethod
    def get_default_replacements(cls, action_name, action_contents):
        """Return alert action parameters derived from REST, in the action.<action_name>.<param_name> format
        ...
        for k, v in action_contents.iteritems():
            if k!='name' and str(v):
    ...
    def get_user_replacements(cls, action_name, action_contents, replacements):
        """Return the valid k=v replacements derived from user input, in the action.<action_name>.<param_name>
        ...
        :param action_contents: A dictionary of alert action contents.
        :type action_contents: dict
        :param replacements: A dictionary of key-value substitutions requested by the user.
        ...
        for k, v in replacements.items():
            if k.startswith(param_template) and k.replace(param_template, '') in action_contents:
                v_params[k] = v
            elif k == 'action_name':
                v_params[k] = v
            else:
                raise ModularActionAdhocException('Invalid parameter for adhoc modular action.')
    ...
    def build_command(self, action_name, token_map, session_key):
        ...
        # Perform standard token replacements for any values that are never present in an adhoc invocation
        # (e.g., "name_hash").
        command = self.replace_standard_tokens(action_contents['command'])

        # Validate the user-provided replacement strings. May raise ModularActionAdhocException.
        user_replacements = self.get_user_replacements(action_name, action_contents, token_map)

        # Obtain the default alert action parameters.
        default_replacements = self.get_default_replacements(action_name, action_contents)
        ...
        # Substitute tokens using default values (unused replacements don't matter)
        command, _ = self.replace_event_tokens(command, default_replacements, substitute_defaults=True)
    ...
    def _normalize_search(search, alert_cmd):
        ...
        :type search: str
        :param alert_cmd: An alert action command string, with all token substitutions already performed.
        :type alert_cmd: str
    ...
    def _parse_search(search, session_key):
        ...
        Note: We intentionally avoid the similar splunk.search.Parser.parseSearch() method because of the excessive
        number of imports used in that library.
    ...
    def _dispatch_search(search, namespace, owner, session_key, dispatch_args=None):
        ...
        Note: We specifically avoid the similar splunk.search.dispatch() method because of the
        excessive number of imports used in that library.
        """
        ...
        if namespace and owner:
            uri = '/servicesNS/%s/%s/search/jobs' % (owner, namespace)
        else:
            uri = 'search/jobs'
    ...
    def handle(self, args):
        ...
        try:
            ...
            msg = 'ActiveResponseException: %s' % e.message
    ...
    def handle_post(self, args):
        ...
        # Retrieve arguments. Everything not popped from form_args is assumed to be a key=value replacement string
        # that will be passed to validate_replacements().
        ...
        # If the handler was invoked with a namespace, both namespace and owner will be used when dispatching search.
        namespace = args.pop('ns', {}).get('app')
        owner = args['session']['user']

        # Earliest and latest used only in dispatching the search, not in any token substitutions.
        ...
        if not all([action_name, search, session_key]):
            raise ModularActionAdhocException('Missing parameters for adhoc modular action.')
        ...
        # Attempt to parse and dispatch the search (may raise ModularActionAdhocException)
        if self._parse_search(finalized_cmd, session_key):
            sid = self._dispatch_search(finalized_cmd, namespace, owner, session_key, dispatch_args)
