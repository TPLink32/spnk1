...
class ConfigApp(admin.MConfigHandler):
  ...
  def setup(self):
    if self.requestedAction == admin.ACTION_EDIT:
      for arg in ['username', 'password',
                  ...
                  'log_individual_findings', 'log_webapp_summary']:
    ...
    if None != confDict:
      for stanza, settings in confDict.items():
        for key, val in settings.items():
            if key in ['log_detections', 'use_proxy', 'enable_debug', 'use_multi_threading', 'use_multi_threading_for_was', 'log_individual_findings', 'log_webapp_summary']:
                pass
            if key in ['username', 'password', 'api_server', 'detection_params', 'extra_was_params', 'proxy_server', 'proxy_user', 'proxy_password'] and val in [None, '']:
                val = ''
            if key in ['minimum_qid'] and val in [None, '']:
                val = '0'
            if key in ['num_threads', 'num_threads_for_was'] and val in [None, '']:
                val = '1'
  ...
  def handleEdit(self, confInfo):
    ...
    if self.callerArgs.data['extra_was_params'][0] in [None, '']:
    ...
    if self.callerArgs.data['num_threads_for_was'][0] in [None, '']:
        self.callerArgs.data['num_threads_for_was'][0] = '1'

    if int(self.callerArgs.data['num_threads_for_was'][0]) < 0 or int(self.callerArgs.data['num_threads_for_was'][0]) > 10:
        self.callerArgs.data['num_threads_for_was'][0] = '1'
